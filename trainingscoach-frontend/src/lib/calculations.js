 ...
 */
export function computeSessionRpe(log) {
  if (!log) return null;
 ...
 */
export function computeWeeklyStrengthLoad(workoutLogs, weeks = 12) {
  if (!workoutLogs || workoutLogs.length === 0) return null;
 ...
 */
export function computeHistogram(timeSec, values, binSize = 1) {
...
export function histogramTotalSeconds(hist) {
  if (!hist) return 0;
 ...
 */
export function timeInZones(hist, zones, lowerKey, upperKey) {
...
export const timeInHrZones = (hist, hrZones) => timeInZones(hist, hrZones, "vanBpm", "totBpm");
export const timeInPowerZones = (hist, powerZones) => timeInZones(hist, powerZones, "vanW", "totW");

/** Durations (seconds) the power curve is sampled at — the conventional set. */
export const POWER_CURVE_DURATIONS = [1, 5, 15, 30, 60, 120, 300, 480, 720, 1200, 1800, 3600];
 ...
 */
export function computePowerCurve(timeSec, watts) {
 ...
 */
export function estimateFtpFromCurve(curve) {
...
export function mergePowerCurves(curves) {
  const merged = {};
