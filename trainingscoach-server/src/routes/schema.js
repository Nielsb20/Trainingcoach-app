...
function getFullSchema() {
  ...
  return {
    ...
    cardioDays: cardioDays.map((c) => ({ id: c.id, weekday: c.weekday, type: c.type, notes: c.notes })),
...
router.put("/", (req, res) => {
  ...
  const replaceAll = db.transaction(() => {
    ...
    const insertCardioDay = db.prepare("INSERT INTO schema_cardio_days (id, weekday, type, notes) VALUES (?, ?, ?, ?)");
    cardioDays.forEach((c) => insertCardioDay.run(c.id, c.weekday, c.type, c.notes || null));
